...
template <class Data>
void _enqueue(Queue<Data>* queue, Data data)
{
    try
    {
        queue->enqueue(data);
...
template <class Data>
bool _dequeue(Queue<Data>* queue, Data& outData)
{
    try
    {
...
template <class Data>
bool _front(Queue<Data>* queue, Data& outData)
{
    try
    {
        outData = queue->front();
...
template <class Data>
bool _back(Queue<Data>* queue, Data& outData)
{
    try
    {
        outData = queue->back();
...
template <class T>
void testQueueFunctions(Queue<T>* queue, T* enqueueArray, size_t enqueueArraySize){
    ...
    if(FIXED_MAX_QUEUE_SIZE)
    {
        ...
        while(!queue->isFull())
        {
            if (index > enqueueArraySize -1) index = 0;
...
int main(int argc, char* argv[])
{
    ...
    {
        integerQueue = new LinkedListQueue<int>(FIXED_MAX_QUEUE_SIZE, MAX_QUEUE_SIZE);
        floatQueue = new LinkedListQueue<float>(FIXED_MAX_QUEUE_SIZE, MAX_QUEUE_SIZE);
        charQueue = new LinkedListQueue<char>(FIXED_MAX_QUEUE_SIZE, MAX_QUEUE_SIZE);
    }
    else
    {
        integerQueue = new ArrayQueue<int>(MAX_QUEUE_SIZE);
        floatQueue = new ArrayQueue<float>(MAX_QUEUE_SIZE);
        charQueue = new ArrayQueue<char>(MAX_QUEUE_SIZE);
    }
    ...
    delete integerQueue, floatQueue, charQueue;
